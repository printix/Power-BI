<#
    ...
    .DESCRIPTION
    This script will extract relevant data from the Printix API (for all tenants you have access to) which in turn will upload the extracted data to a specified Azure Storage account.
    ...
    .EXAMPLE
    Get-PrintixData -PartnerID '45dbbbbb-62929-1111-4444-15153b055555' -ClientCredentialsName 'PrintixClientCredentials' -DeleteExtractedData $true -DaysToExtract 5

    This will get all tenants extract data for the last 5 months, for the given partnerID, using the PrintixClientCredentials credential asset before deleting the extracted data.
...
#Fill in the TenantDomain, StorageAccount name, resource group and Container name where you want to store the processed printix data, for use with Power bi, for a specific customer.
...
Try {
    ...
    #Only work with tenants that are specified in the $StorageMapping object.
