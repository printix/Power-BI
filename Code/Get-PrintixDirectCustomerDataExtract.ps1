<#
    ...
    .DESCRIPTION
    This script will extract relevant data from the Printix API which in turn will upload the extracted data to a specificed Azure Storage account.
    ...
    .EXAMPLE
    Get-PrintixData -PrintixCustomerID '45dbbbbb-62929-1111-4444-15153b055555' -ClientCredentialsName 'PrintixClientCredentails' -DeleteExtractedData $true -DaysToExtract 5

    This will get all tenants extract data for the last 5 months, for the given CustomerID, using the PrintixClientCredentails credential asset before deleting the extracted data.

#>

param (
    [Parameter(Mandatory = $true, ValueFromPipelineByPropertyName = $true, Position = 0, HelpMessage = 'This is the Printix Customer ID')]
    [String] $PrintixCustomerID = '',
...
#Fill in the TenantDomain, StorageAccount name, resourcegroup and Container name where you want to store the processed data.
[PSCustomObject]$StorageMapping = @{
    ...
    Tenants                                         = @(
        [PSCustomObject]@{
            TenantDomain   = 'TENANTNAME.printix.net'
            StorageAccount = 'powerbiprintixTENANTNAME'
